...
/// @abstract 连麦时，远端用户（以 userID 标识）的视频首帧解码后的回调，如果需要显示，则该需要返回含 renderView 的 PLRTCVideoRender 对象，本接口在主队列中回调
...
///           - (PLRTCVideoRender *)RTCStreamingSession:(PLRTCStreamingSession *)session
...
/// @abstract 连麦时，远端用户（以 userID 标识）取消发布视频数据的回调
- (void)RTCStreamingSession:(PLRTCStreamingSession *)session didUnpublishVideoOfUserID:(NSString *)userID;
 ...
 *
 * @discussion 开始连麦后，音视频会发布到房间中，同时拉取房间中的音视频流。可通过 PLMediaStreamingSessionDelegate 的回调得到连麦的状态。
 *
 * @warning 首次调用该接口前需先调用 - (void)setWithServerRegionID:(PLRTC_SERVER_REGION)serverRegionID serverRegionName:(NSString *)serverRegionName; 接口设置连麦参数。
 */
 ...
 * 结束连麦
 *
 * @discussion 结束连麦后，会停止推送本地音视频流，同时停止拉取房间中的音视频流。。
 */
- (void)stopConference;
